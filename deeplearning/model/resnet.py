...
class ResNet_Bottleneck_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 50:
            ...
            resnet.load_state_dict(torch.load("/root/deeplearning/pretrained_models/resnet/resnet50-19c8e357.pth"))
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/root/deeplearning/pretrained_models/resnet/resnet101-5d3b4d8f.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/root/deeplearning/pretrained_models/resnet/resnet152-b121ed2d.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/root/deeplearning/pretrained_models/resnet/resnet18-5c106cde.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/root/deeplearning/pretrained_models/resnet/resnet34-333f7ec4.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
